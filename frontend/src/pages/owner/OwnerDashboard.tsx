...
const MenuItemCard: React.FC<MenuItemCardProps> = ({ item, onEdit, onDelete, onToggleAvailability }) => {
  const { name, description, price, isAvailable, preparationTime } = item;
...
export function MenuManagement() {
  ...
  // Derive initial form data from editingItem or use default
  const initialFormData = editingItem ? {
      name: editingItem.name,
      description: editingItem.description,
      price: editingItem.price,
      // Ensure preparationTime is always a number, defaulting to 15
      preparationTime: editingItem.preparationTime || 15, 
      isAvailable: editingItem.isAvailable,
  } : DEFAULT_FORM_DATA;


  // --- Data Fetching and CRUD Handlers ---
  ...
  const loadMenuItems = useCallback(async () => {
    try {
      ...
      // Map and ensure preparationTime exists for the interface
  ...
  const toggleAvailability = async (item: MenuItem) => {
    try {
      ...
      setMenuItems(prevItems => 
        prevItems.map(i => i.id === item.id ? { ...i, isAvailable: !i.isAvailable } : i)
      );
    } catch (error) {
  ...
  const handleFormSave = () => {
    setShowForm(false);
    setEditingItem(null);
    loadMenuItems(); // Full reload after successful save
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {/* Add/Edit Form (Rendered as a separate component) */}
